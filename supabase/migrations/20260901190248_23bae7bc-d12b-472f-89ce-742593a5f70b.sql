...
-- ============ STATUS SYNC: preserve manual statuses ============
...
BEGIN
  ...
  SET status = CASE
      WHEN EXISTS (SELECT 1 FROM public.leases l WHERE l.unit_id = u.id AND l.status = 'active') THEN 'occupied'
...
DROP POLICY IF EXISTS "staff manage property images" ON public.property_images;
CREATE POLICY "staff manage property images" ON public.property_images
  FOR ALL TO authenticated
  USING (public.is_staff(auth.uid()))
  WITH CHECK (public.is_staff(auth.uid()));

DROP POLICY IF EXISTS "public read unit images" ON public.unit_images;
...
DROP POLICY IF EXISTS "staff manage unit images" ON public.unit_images;
CREATE POLICY "staff manage unit images" ON public.unit_images
  FOR ALL TO authenticated
  USING (public.is_staff(auth.uid()))
  WITH CHECK (public.is_staff(auth.uid()));

DROP POLICY IF EXISTS "public read published units" ON public.property_units;
...
DROP POLICY IF EXISTS "properties staff manage units" ON public.property_units;
...
-- ============ BACKFILL PROPERTY GALLERY (idempotent, non-destructive) ============
INSERT INTO public.property_images (property_id, path, sort_order, is_cover)
SELECT p.id, img.path, (img.ord - 1)::int,
       (p.featured_image IS NOT NULL AND p.featured_image = img.path)
FROM public.properties p
CROSS JOIN LATERAL unnest(COALESCE(p.images, '{}'::text[])) WITH ORDINALITY AS img(path, ord)
WHERE img.path IS NOT NULL AND img.path <> ''
  AND NOT EXISTS (SELECT 1 FROM public.property_images pi
                  WHERE pi.property_id = p.id AND pi.path = img.path);
